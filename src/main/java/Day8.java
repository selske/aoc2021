...
class Day8 extends AocDay<List<Day8.Signal>> {
...
//            """;
    ...
    @Override
    List<Signal> prepareInput() throws Exception {
        return INPUT.lines()
                .map(line -> {
                    var parts = line.split(" \\| ");
                    var signalPatterns = List.of(parts[0].split(" "));
                    var outputValues = List.of(parts[1].split(" "));
                    return new Signal(signalPatterns, outputValues);
                })
                .toList();
    }

    @Override
    String part1(List<Signal> input) throws Exception {
        return input.stream()
                       .flatMap(signal -> signal.outputValues.stream())
    ...
    @Override
    String part2(List<Signal> input) throws Exception {
        return input.stream()
                       .mapToInt(this::solve)
                       .sum() + "";
    }

    private static final int[] segmentMasks = new int[]{
            0b01000000,
            0b00100000,
            0b00010000,
            0b00001000,
    ...
    private static final Map<Integer, Integer> masks = Map.of(
            0b01110111, 0, // -> 119
            0b00010010, 1, // -> 18
            0b01011101, 2, // -> 93
            0b01011011, 3, // -> 91
    ...
    List<List<Character>> permutations = CollectionUtils.permutations(IntStream.rangeClosed('a', 'g')
                    .mapToObj(i -> (char) i)
                    .toList()).stream()
            .toList();

    private int solve(Signal signal) {
        Stream<List<Character>> possibleSolutions = permutations.stream();
        for (String signalPattern : signal.signalPatterns()) {
            possibleSolutions = possibleSolutions
                    .parallel()
                    .filter(permutation -> masks.containsKey(toMask(signalPattern, permutation)));
        }
        var solution = possibleSolutions.findAny().orElseThrow();

        return Integer.parseInt(
                signal.outputValues().stream()
                        .map(outputValue -> masks.get(toMask(outputValue, solution)) + "")
                        .collect(joining())
        );
    }

    private int toMask(String signalPattern, List<Character> pattern) {
        return signalPattern.chars()
                .map(o -> pattern.indexOf((char) o))
                .map(i -> segmentMasks[i])
                .reduce((a, b) -> a | b)
                .orElseThrow();
    }

    record Signal(List<String> signalPatterns, List<String> outputValues) {
